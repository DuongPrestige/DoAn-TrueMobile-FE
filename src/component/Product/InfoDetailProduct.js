...
function InfoDetailProduct(props) {
    ...
    let handleSelectDetail = (event) => {
        ...
        if (productDetail[event.target.value] && productDetail[event.target.value].productDetailConfig.length > 0) {
            setactiveLinkId(productDetail[event.target.value].productDetailConfig[0].id)
            setquantity(productDetail[event.target.value].productDetailConfig[0].stock)
            props.sendDataFromInforDetail(productDetail[event.target.value].productDetailConfig[0])
        }
