...
const AddBrand = (props) => {
  ...
  let handleSaveCategory = async () => {
    if (isActionADD === true) {
      ...
      } else toast.error("Thêm hãng điện thoại thất bại");
    } else {
      ...
      if (res && res.errCode === 0) {
        toast.success("Cập hãng điện thoại thành công");
      } else if (res && res.errCode === 2) {
        toast.error(res.errMessage);
      } else toast.error("Cập hãng điện thoại thất bại");
    }
  };
    ...
    <div className="container-fluid px-4">
      ...
      <div className="card mb-4">
        <div className="card-header">
          ...
          {isActionADD === true
            ? "Thêm mới hãng điện thoại"
            : "Cập nhật thông tin hãng điện thoại"}
        </div>
        <div className="card-body">
          <form>
            <div className="form-row">
              <div className="form-group col-md-6">
                <label htmlFor="inputEmail4">Tên hãng điện thoại</label>
                <input
                  type="text"
                  value={inputValues.value}
                  name="value"
