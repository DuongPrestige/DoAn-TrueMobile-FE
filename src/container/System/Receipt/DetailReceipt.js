...
const DetailReceipt = (props) => {
  ...
  const handleOnChangeProduct = (event) => {
    ...
    for (let i = 0; i < dataProduct.length; i++) {
      if (dataProduct[i].id == value) {
        ...
        setdataProductDetailSize(
          dataProduct[i].productDetail[0].productdetailconfig
        );
        setproductDetailSizeId(
          dataProduct[i].productDetail[0].productdetailconfig[0].id
        );
      }
    }
  };
  let handleOnChangeProductDetail = (event) => {
    ...
    for (let i = 0; i < dataProductDetail.length; i++) {
      if (dataProductDetail[i].id == value) {
        setdataProductDetailSize(dataProductDetail[i].productdetailconfig);
        setproductDetailSizeId(dataProductDetail[i].productdetailconfig[0].id);
      }
    }
  };
  let handleSaveReceiptDetail = async () => {
    let res = await createNewReceiptDetailService({
      receiptId: id,
      productdetailconfigId: productDetailSizeId,
        ...
        <div className="card-body">
          <form>
            <div className="form-row">
              ...
              <div className="form-group col-md-4">
                <label htmlFor="inputEmail4">Rom sản phẩm</label>
                <select
              ...
              <tbody>
                {dataReceiptDetail &&
                  ...
                  dataReceiptDetail.map((item, index) => {
                    let name = `${item.productData.name} - ${item.productDetailData.nameDetail} - ${item.productDetailSizeData.romData.value}`;
                    return (
                      <tr key={index}>
                        <td>{index + 1}</td>
