...
import {
  ...
  getAllProductDetailConfigByIdService,
...
const ManageProductImage = () => {
  ...
  //edit day problem here
  let handleEditProductSize = (id) => {
    setproductSizeId(id);
    setisOpenModalSize(true);
  };

  // let handleEditProductSize = async (data) => {
  //   setisOpenModalSize(true);
  //   let response = await UpdateProductDetailSizeService({
  //     data: {
  //       id: data.id,
  ...
  let handleDeleteProductSize = async (productdetailsizeId) => {
    ...
    if (response && response.errCode === 0) {
      toast.success("Xóa thông tin chi tiết thành công !");
      ...
      toast.error("Xóa thông tin chi tiết thất bại!");
    }
  };
