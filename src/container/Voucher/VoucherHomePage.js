  ...
  let sendDataFromVoucherItem = async (id) => {
    if (user && user.id) {
      ...
      if (res && res.errCode === 0) {
        toast.success("Lụm mã voucher thành công !");
        await fetchData();
      } else {
          ...
          <div class="container">
            <div class="banner_content d-md-flex justify-content-between align-items-center">
              <div class="mb-3 mb-md-0">
                <h2>Khuyến mãi</h2>
                <p>Hãy nhanh tay lưu mã giảm giá để nhận ưu đãi hấp dẫn</p>
              </div>
              <div class="page_link">
                <Link to={"/"}>Trang chủ</Link>
                <Link to={"/voucher"}>Khuyến mãi</Link>
              </div>
            </div>
          </div>
        </div>
            ...
            dataVoucher.map((item, index) => {
              ...
              return (
                <VoucherItem
                  ...
                  maxValue={MaxValue}
                  usedAmount={
                    Math.round(((item.usedAmount * 100) / item.amount) * 10) /
                    10
                  }
                  typeVoucher={percent}
                />
